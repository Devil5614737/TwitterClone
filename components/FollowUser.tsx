...
export const FollowUser = ({ image, username,id,handleFollow,user,followers }: PropsI) => {
  const {currentUser}=useContext<AuthContextI>(AuthContext);




  let followed=followers.find((item:any)=>item.user.uid===currentUser?.uid as string);



  return (
   <div onClick={()=>handleFollow(id,user)} className="my-5 flex items-center justify-between">
